...
class PxGridControl:
    ...
    def get_endpoints(self, *, start_timestamp=ENDPOINT_BULK_START,
        ...
        logger.warning(
            ...
            "resolved and the query is authorized, but ISE has no pxGrid endpoint "
            "snapshot to return. This is expected on ISE 3.3 and common elsewhere when "
            "ISE is not publishing endpoint context. ERS remains the baseline endpoint "
            "inventory/profile source; pxGrid endpoint snapshots only enrich MFC model/OS, "
            "Secure Client, and posture-report fields when ISE actually publishes them. "
            "Check the pxGrid client group grants EndpointService (com.cisco.ise.endpoint) "
            "and that endpoints have live pxGrid-published context on releases where "
            "getEndpoints is expected to work.")
        return []
